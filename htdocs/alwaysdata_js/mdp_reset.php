...
if ($chaine) {
 ...
 $res = mysqli_query($con, "SELECT * FROM assoagricoulis_site.reset_mdp WHERE chaine='$chaine';");
 ...
 if ($res) {
  ...
  if ($time - $time_reset < 900 ) {
   ...
   if (isset($mdp) && $mdp == $remdp) {
    ...
    $res = mysqli_query($con, "UPDATE assoagricoulis_site.utilisateurs SET mdp='$mdp' WHERE id='$id';");
    if ($res) {
     echo "<p>Réinitialisation réussie !<br><a href='index.php'>Connectez-vous</a>";
     mysqli_query($con, "DELETE FROM assoagricoulis_site.reset_mdp WHERE chaine='$chaine'");
     }
