...
<script>
function afficher_arborescence(elt, tabs = "") {
  if (elt.getAttribute('id') === "FIN") {
    return;
  }
  document.writeln(tabs + "{ tag: " + elt.localName + ", children: ");
  if (elt.children.length > 0) {
    document.writeln('[<br id="FIN">');
    var enf = elt.children
    var i;
    for (i = 0; i < enf.length; i++) {
      afficher_arborescence(enf[i], tabs + "&nbsp&nbsp&nbsp&nbsp");
    }
    //document.writeln('<br id="FIN">');
    document.writeln(tabs + ']&nbsp}<br id="FIN">');
  }
  else {
    document.writeln('null }<br id="FIN">');
  }
}

afficher_arborescence(document.documentElement);
</script>
</html>
