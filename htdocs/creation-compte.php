...
if ($login != NULL and $mdp != NULL and $remdp != NULL and $mail != NULL) {
  ...
  $resmail = mysqli_query($con, "SELECT * FROM agricoulis.utilisateurs WHERE mail='$mail';");
  $reslog = mysqli_query($con, "SELECT * FROM agricoulis.utilisateurs WHERE login='$login';");
  ...
  //si tout est ok on ajoute le nouvel utilisateur à la base après avoir chiffré le mot de passe
    ...
    mysqli_query($con, "INSERT INTO agricoulis.utilisateurs VALUES ('$login', '$mail', '$mdp_hash', NULL);");
    session_start();
